...
export default async function PostPage({ params }: Props) {
  ...
  return (
    <div style={{ padding: 32, fontFamily: 'monospace' }}>
      <h1>POST_DEBUG_QUERIES</h1>
      <p>title: {post.title}</p>
      <p>board: {config?.label}</p>
      <p>cat: {category?.name}</p>
      <p>comments: {comments?.length ?? 'null'}</p>
      <p>empathyCount: {String(empathyCount)}</p>
      <p>hasEmpathy: {String(hasEmpathy)}</p>
      <p>relatedPosts: {relatedPosts?.length ?? 'null'}</p>
    </div>
  );
}
